...
# src/cli.ts pool index writes SHA256SUMS over exactly the pool it
...
# STALE, sense 3. Every archive in the pool is a row of src/cli.ts pool rows: a
...
bash "$REPO_ROOT/bin/bun.sh" src/cli.ts pool rows --arch "$MICA_ARCH" >"$OUT_DIR/pool-rows.tsv" ||
...
# intentions.
#
# EVERY COLUMN IS READ OUT OF THE POOL INDEX, which src/cli.ts pool index
...
# database already carries at the point the finalizer purges it.
{
    ...
    echo "# Read out of $POOL_DIR/manifest.txt, which src/cli.ts pool index"
