...
| `IMAGE_KINDS` | optional; a subset of the kinds the board's `images.tsv` declares (default: all of them; `disk` is always one). Each is packed and verified by the board's packer (`src/product/image-kinds.ts`) |
