...
# locks/upstream.lock.
#
#   bash tools/locks.sh check     the file rules of every lock and pin, offline
#   bash tools/locks.sh verify    check, then every pinned lock is its release's,
...
case "${1-}" in
check) [ "$#" -eq 1 ] || die "usage: bash tools/locks.sh check | verify"; check ;;
verify) [ "$#" -eq 1 ] || die "usage: bash tools/locks.sh check | verify"; check; verify ;;
*) die "usage: bash tools/locks.sh check | verify" ;;
esac
