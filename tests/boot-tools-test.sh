...
#
#   bash tests/boot-tools-test.sh      (make os-boot-test; after make os-boot-tools and make os-pool)
set -euo pipefail
cd "$(dirname "$0")/.."
REPO_ROOT="$(pwd)"
...
run /src/tests/boot-initramfs-test.sh /src
run /src/tests/boot-compression-test.sh
