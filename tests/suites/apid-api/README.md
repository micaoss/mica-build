# `tests/suites/apid-api` — the apid black-box HTTP suite

A bun + TypeScript suite that talks to **apid** in a booted Mica OS guest over the
...
| `APID_ADMIN_PASSWORD` | no | `mica-e2e-admin-pw` | password sent to `/api/v1/setup` and `/api/v1/session` |
| `APID_HOSTNAME_TARGET` | no | `mica-e2e-renamed` | hostname sent to `/api/v1/setup` and read back through `/api` |
...
```sh
docker run --rm -v "$(git rev-parse --show-toplevel):/w" -w /w/tests/suites/apid-api \
