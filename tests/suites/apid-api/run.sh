...
#   bash tests/suites/apid-api/run.sh --dry-run
#
...
if ! RUN_DIR_REAL="$(readlink -f "${RUN_DIR}")"; then
    echo "FAIL: ${OUT_DIR} does not exist, so there is no image to boot; this harness builds nothing. Build a complete image with bin/bun.sh src/cli.ts components image" >&2
    exit 1
fi
...
# nobody chose. Same rule, and the same two cases, as bin/bun.sh src/cli.ts's.
case "${DOCKER_HOST:-}" in
...
if [ ! -e "${IMG}" ]; then
    fail "image ${IMG##*/} is missing; this harness builds nothing. Build a complete image with bin/bun.sh src/cli.ts components image"
    finish
fi
...
if [ ! -f "${SCRIPT_DIR}/src/main.ts" ]; then
    ...
    # the suite.
    note "tests/suites/apid-api/src/main.ts does not exist yet; the suite step will FAIL loudly rather than be skipped quietly"
fi
...
if [ "${DRY_RUN}" -eq 1 ]; then
    ...
    note "would seed     tests/suites/apid-api/guest/m7-net-smoke.sh -> DATA:${SMOKE_IN_GUEST} (phase 05c)"
    ...
    note "would run      docker run --network ${NET} -v ${REPO_ROOT}:/w -v ${OUT_REAL}:/w/_out -w /w/tests/suites/apid-api ${BUN_IMAGE} bun run src/main.ts"
...
run_suite() {
    ...
    if [ ! -f "${SCRIPT_DIR}/src/main.ts" ]; then
        fail "[${label}] the suite entry point tests/suites/apid-api/src/main.ts does not exist, so nothing was asserted about apid"
        SUITE_RC=1
        return 0
    fi
    ...
    set +e
    docker run --rm --label ai-agent=true --network "${NET}" \
        -v "${REPO_ROOT}:/w" -v "${OUT_REAL}:/w/_out" -w /w/tests/suites/apid-api \
        -e APID_HOST="${ip}" \
...
# later. `<board>-mica-latest.img` is a symlink and its target changes under it
