...
try {
  ...
  const posted = await client.post(
    '/echo-form',
    { password: 'pw', hostname: 'mica', dhcp: checkbox(false), dns: UNCHECKED },
    { scheme: 'http' },
  )
  outer.check(
    posted.body === 'password=pw&hostname=mica',
    'an unticked checkbox is omitted from the form body, as a browser omits it',
    [
      `expected: the body "password=pw&hostname=mica"`,
  ...
  outer.check(
    ...
    && defaults.adminPassword === 'mica-e2e-admin-pw'
    && defaults.hostnameTarget === 'mica-e2e-renamed'
    && defaults.phases === undefined,
    'the documented defaults are the defaults',
    [
      `expected: 18443/18080, mica-e2e-admin-pw, mica-e2e-renamed, all phases`,
  ...
  const nameHosts = ['mica.local', 'localhost', 'example.test']
  ...
  for (const [image, certificate, append] of [[undefined, 'cert', undefined], ['disk', undefined, undefined], ['disk', 'cert', 'init=/bin/sh']]) {
    let refused = false
    try { requireSignedInputs(image, certificate, append) }
    catch { refused = true }
