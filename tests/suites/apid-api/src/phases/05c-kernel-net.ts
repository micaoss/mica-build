 ...
 * unpacked squashfs; src/verify/checks-kernel.ts was deleted on 2026-09-09 in
 ...
 *
 * THE GUEST SIDE IS NOT HERE. `tests/suites/apid-api/guest/m7-net-smoke.sh` runs inside
...
const phase: Phase = {
  ...
  assumes:
    'the guest booted from a disk that run.sh seeded with tests/suites/apid-api/guest/m7-net-smoke.sh '
    ...
    if (found.size === 0) {
      ...
      report.fail(
        ...
        [
          ...
          'actual:   none. The guest never ran tests/suites/apid-api/guest/m7-net-smoke.sh, so NOTHING',
