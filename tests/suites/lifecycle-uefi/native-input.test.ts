...
import { kernelExecutables } from '../../../src/image/kernel-package.ts'
...
function fixture(script: string, board: string, kind: string) {
  const work = mkdtempSync(join(tmpdir(), 'mica-file-ab-native-')); owned.push(work)
  ...
  const args = script === 'build.ts'
    ? [join(work, 'output'), board, kernel, cert, signingKey, runkit, root]
    : script === 'update.ts'
      ? [baseline, cert, signingKey, '3', kind, root, kernel, runkit]
