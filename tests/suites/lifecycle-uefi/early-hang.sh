...
printf 'Evidence: %s\n' "$work"
timeout -k 20 700 bun tests/suites/lifecycle-uefi/update.ts "$out" "$cert" "$key" 7 kernel "$out/root" "$out/kernel" "$runkit"
timeout -k 15 450 docker run --rm --label ai-agent=true --network traefik \
    -v "$out:/w" -v "$PWD/tests/suites/lifecycle-uefi:/harness:ro" ai-agent/mica-p2-lab \
...
bash tests/suites/lifecycle-uefi/shutdown-check.sh "$out/install.log"
bad=$(cat "$out/offline/expected-id")
mv "$out/offline" "$out/updates/7/installed-media"
python3 - "$out/boot.sh" <<'PY'
from pathlib import Path
import sys
s=Path('tests/suites/lifecycle-uefi/boot.sh').read_text()
...
for attempt in 1 2 3; do
    ...
    timeout -k 15 450 docker run --rm --label ai-agent=true --network traefik \
        -v "$out:/w" -v "$PWD/tests/suites/lifecycle-uefi:/harness:ro" ai-agent/mica-p2-lab \
        bun /harness/qmp-boot.ts "/w/events-$attempt.jsonl" bash /w/boot.sh image/disk.img writable 400 "$arch" > "$out/attempt-$attempt.log" 2>&1
...
PY
    ...
    grep -F "mica-$bad+$((3-attempt))-$attempt.conf" "$out/entries-$attempt.txt"
done
timeout -k 15 450 docker run --rm --label ai-agent=true --network traefik \
    -v "$out:/w" -v "$PWD/tests/suites/lifecycle-uefi:/harness:ro" ai-agent/mica-p2-lab \
