import { builderImagesAt } from '../../../src/image/images.ts'
import { createHash } from 'node:crypto'
import { lstatSync, readFileSync, writeFileSync } from 'node:fs'
import { join, resolve } from 'node:path'
import { canonicalJson } from '../../../src/image/components.ts'
import { assembleRelease, fileSha256, gateRelease, type ReleaseInputs } from '../../../src/image/release-manifest.ts'
import { sourceIdentity } from '../../../src/image/release-cli.ts'
import { REPO_ROOT } from '../../../src/image/paths.ts'
...
export async function acceptProvenance(inputs: ReleaseInputs, checkout = REPO_ROOT) {
  ...
  const builderImages = builderImagesAt(checkout)
  ...
  const record = {
    schema: 'mica/non-publication-artifact-acceptance/v1', result: 'NON_PUBLICATION_ARTIFACT_ACCEPTANCE',
...
if (import.meta.main) {
  try {
    ...
    if (!input || extra.length) throw new Error('Usage: tests/suites/lifecycle-uefi/bun.sh tests/suites/lifecycle-uefi/provenance-acceptance.ts INPUTS_JSON')
