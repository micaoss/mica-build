...
3. **The class `removed` may never be sanctioned, for any path.** A removed path
   ...
   which reaches the image because `stages/compose/scripts/overlay-install.sh` copies
...
empty set with an empty set, and `tests/suites/factory-root-gate` says the same
...
the day the account was made. The mica-owned accounts do NOT move -- `Mica OS`,
`mica-mqttd` and `mica-mqtt-broker` all read 18262 (2020-01-01) in both builds,
because `stages/compose/scripts/account-*.sh` pins them with `chage -d`. The
...
- **`/etc/systemd/system/multi-user.target.wants/ssh.service`, class `added`.**
  ...
  `stages/compose/scripts/network-and-ssh-units.sh:22` removes the link
...
- **`/run/crun`, class `removed`.** Build residue: the chain EXERCISES the
  engine during assembly (`stages/compose/scripts/podman-exercise.sh` runs `crun`),
...
- **`/usr/sbin/policy-rc.d`, class `removed`.** Not ours and never was: the
  ...
  `stages/compose/scripts/pack-strip-build-residue.sh`, which also carries the sshd
...
- **`/tmp/pool.names`, class `added`.** A scratch file the composer wrote and
  ...
  `compose-install.sh`, which now writes it under the removed `/mica-compose` and
...
- It compares the **file tree of the packed root** and nothing else. Anything
  ...
  `tests/suites/factory-root-gate` is what ties the exported root back to the
...
- status: active
- expect-diff: |
    24 -mica-mqttd:x:970:970:mica MQTT bridge:/nonexistent:/usr/sbin/nologin
    24 +mica-mqtt-broker:x:969:969:mica MQTT broker:/nonexistent:/usr/sbin/nologin
    25 -mica-mqtt-broker:x:969:969:mica MQTT broker:/nonexistent:/usr/sbin/nologin
    25 +mica-mqttd:x:970:970:mica MQTT bridge:/nonexistent:/usr/sbin/nologin
- reason: The two service accounts mica-mqttd (970) and mica-mqtt-broker (969) are created in the opposite order on the two paths: the chain runs stages/compose/scripts/account-mica-mqttd.sh and account-mica-mqtt-broker.sh in stage order, and the composer gets dpkg's configuration order, which set up mica-mqtt-broker first. Both orders are deterministic within their own path and neither is wrong, so this is an artefact of having TWO paths and it cannot exist once there is one. Measured, not argued: `diff <(sort A) <(sort B)` over this file is EMPTY, so both roots hold the same entries with the same uid and gid, and only the position of two lines differs. The expected diff is what makes this safe to sanction at all. A bare `content` stanza here would equally cover an account VANISHING from the composed root, which is the failure this comparison exists to catch; with the diff named, a vanished account shifts every following line, a third line moving adds rows, and a field changing inside a transposed line changes one -- each produces a different canonical diff and each still FAILS.
...
- expect-diff: |
    24 -mica-mqttd:x:970:970:mica MQTT bridge:/nonexistent:/usr/sbin/nologin
    24 +mica-mqtt-broker:x:969:969:mica MQTT broker:/nonexistent:/usr/sbin/nologin
- reason: The two service accounts mica-mqttd (970) and mica-mqtt-broker (969) are created in the opposite order on the two paths: the chain runs stages/compose/scripts/account-mica-mqttd.sh and account-mica-mqtt-broker.sh in stage order, and the composer gets dpkg's configuration order, which set up mica-mqtt-broker first. Both orders are deterministic within their own path and neither is wrong, so this is an artefact of having TWO paths and it cannot exist once there is one. Measured, not argued: `diff <(sort A) <(sort B)` over this file is EMPTY, so both roots hold the same entries with the same uid and gid, and only the position of two lines differs. This is the -backup, which glibc's account tools write as a snapshot of the file BEFORE their last write, so it holds one write less than its live counterpart and the transposition shows as a single replaced line rather than as a pair. Same cause, same two accounts. The expected diff is what makes this safe to sanction at all. A bare `content` stanza here would equally cover an account VANISHING from the composed root, which is the failure this comparison exists to catch; with the diff named, a vanished account shifts every following line, a third line moving adds rows, and a field changing inside a transposed line changes one -- each produces a different canonical diff and each still FAILS.
...
- expect-diff: |
    ...
    51 -mica-mqtt-broker:!::
    51 +mica-mqttd:!::
- reason: The two service accounts mica-mqttd (970) and mica-mqtt-broker (969) are created in the opposite order on the two paths: the chain runs stages/compose/scripts/account-mica-mqttd.sh and account-mica-mqtt-broker.sh in stage order, and the composer gets dpkg's configuration order, which set up mica-mqtt-broker first. Both orders are deterministic within their own path and neither is wrong, so this is an artefact of having TWO paths and it cannot exist once there is one. Measured, not argued: `diff <(sort A) <(sort B)` over this file is EMPTY, so both roots hold the same entries with the same uid and gid, and only the position of two lines differs. This path is one of the three L1 ruled may never carry a bare sanction, and it carries a NARROWED one instead, which subsumes that rule rather than weakening it. The last-change field is pinned into the expected diff as 18262, so the date regression the rule was written about -- Debian's postinst accounts dating themselves with the build day -- changes these rows and fails. Measured before this stanza existed: forcing that field to 18262 on both sides left exactly the transposition below, which is how the date cause and the ordering cause were told apart. stages/compose/scripts/account-pin-shadow-dates.sh in the shared finalizer is what removes the date cause on both paths. The expected diff is what makes this safe to sanction at all. A bare `content` stanza here would equally cover an account VANISHING from the composed root, which is the failure this comparison exists to catch; with the diff named, a vanished account shifts every following line, a third line moving adds rows, and a field changing inside a transposed line changes one -- each produces a different canonical diff and each still FAILS.
