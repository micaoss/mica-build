...
# copied unchanged into tests/fixtures/release-lock/vectors/: every lock, upstream and pins
...
#
#   bash tests/gates/release-lock-test.sh      (make os-release-lock-test; python3, no network)
set -euo pipefail
cd "$(dirname "$0")/../.."
