...
//   - the five boot-logo artefacts move with BOARD_BOOT_LOGO, all or none (logoArtefacts, which
...
/**
 * THE BOOT LOGO'S FIVE ARTEFACTS MOVE TOGETHER. BOARD_BOOT_LOGO is the one switch, and a board either has all five
 ...
 * prepare hook or its kernel Dockerfile), and the two policy files that keep the logo's VT idle. A drop-in that
 * outlives its logo removes a working VT login to protect nothing; a logo with no mask is covered by the first getty
 * anyone enables.
 */
export function logoArtefacts(dir: string): { flag: boolean, have: number } {
  ...
  const mask = join(dir, 'package/overlay/etc/systemd/system/getty@tty1.service')
  try { if (lstatSync(mask).isSymbolicLink()) have++ }
  catch { /* absent */ }
  return { flag: /^BOARD_BOOT_LOGO=1$/m.test(env), have }
}

export const logoAgrees = ({ flag, have }: { flag: boolean, have: number }) => (flag && have === 5) || (!flag && have === 0)
...
export function contract(board: string, root = REPO_ROOT): string[] {
  ...
  if (!logoAgrees(logo)) fail(`${board}: BOARD_BOOT_LOGO=${logo.flag ? 1 : 0} with ${logo.have} of the five logo artefacts present (CONFIG_LOGO, fbcon=logo-pos: with vt.global_cursor_default=0, the mklogo render, the logind drop-in, the getty@tty1 mask). They move together or not at all`)
