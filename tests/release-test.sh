...
PREVIOUS="${HISTORY}/x64.20260914-2042"
mkdir -p "${PREVIOUS}"
...
expect_refusal "a slash between the scope and the stamp" "must be <scope>.<YYYYMMDD-HHMM>" plan x64/20260916-0000
expect_refusal "a slash index tag" "verify-index takes mica.<YYYYMMDD-HHMM>" verify-index mica/20260915-2242
mkdir -p "${HISTORY}/x64.20260915-0100"
sed 's|x64\.20260914-2042|x64/20260914-2042|' "${PREVIOUS}/mica-build.lock" >"${HISTORY}/x64.20260915-0100/mica-build.lock"
(cd "${HISTORY}/x64.20260915-0100" && sha256sum mica-build.lock >SHA256SUMS)
expect_refusal "an earlier release whose lock carries a slash release row" "its mica-build.lock breaks a rule" plan x64-dev.20260916-0000
rm -rf "${HISTORY}/x64.20260915-0100"
# A generation floor for a history this tree no longer reads; it never lowers one.
if [ "$(MICA_RELEASE_GENERATIONS="x64-dev=9 x64-prod=4" release plan x64.20260916-0000 | cut -f1,3)" = "x64-dev	9
x64-prod	4" ]; then
    pass "MICA_RELEASE_GENERATIONS raises a planned generation"
else
    fail "the generation floor: $(MICA_RELEASE_GENERATIONS="x64-dev=9 x64-prod=4" release plan x64.20260916-0000 2>&1 | tail -2)"
fi
MICA_RELEASE_GENERATIONS="x64-dev=3" expect_refusal "a generation floor below the planned generation" \
    "gives x64-dev generation 3, below the planned 4" plan x64.20260916-0000
MICA_RELEASE_GENERATIONS="x64-dev=one" expect_refusal "a generation floor that is no decimal" \
    "each item is <product>=<generation>, a decimal of at least 2" plan x64.20260916-0000
mkdir -p "${HISTORY}/x64.20260916-0000" "${HISTORY}/x64.20260915-0000"
...
printf 'partial\n' >"${HISTORY}/x64.20260915-0000/mica-x64-dev-20260915-0000.img"
...
rm -rf "${HISTORY}/x64.20260916-0000" "${HISTORY}/x64.20260915-0000"
...
IDX="${SCRATCH}/index"
A=x64.20260916-0000
mkdir -p "${IDX}/history/x64.20260916-0000" "${IDX}/downloads/${A}" "${IDX}/assets"
cp "${DIR}/mica-build.lock" "${DIR}/SHA256SUMS" "${IDX}/history/x64.20260916-0000/"
cp -r "${DIR}/assets" "${IDX}/assets/x64.20260916-0000"
...
fabricate() {
    ...
    for f in "${DIR}"/assets/*; do
        cp "${f}" "${IDX}/assets/$1/$(basename "${f}" | sed -e "s/x64-dev/$2/" -e "s/20260916-0000/${stamp}/")"
    done
...
input	mica-build.x64-prod	20260916-0100	$(sha "${IDX}/history/x64-prod.20260916-0100/SHA256SUMS")" ] &&
...
rm -rf "${IDX}/history/x64.20260916-0200"
...
mv "${IDX}/history/x64-prod.20260916-0100" "${IDX}/aside/history/"
mv "${IDX}/assets/x64-prod.20260916-0100" "${IDX}/aside/assets/"
rm -rf "${IDX}/history/x64.20260916-0000"
if out="$(index_env MICA_INDEX_STAMP=20260918-0100 MICA_INDEX_OUT="${IDX}/inc" bash tools/release.sh index --dry-run "${B}" 2>&1)" &&
    ...
    [ "$(grep $'^input\tmica-build.x64\t' "${IDX}/inc/mica-build.lock" | cut -f3,4)" = "20260918-0000	$(sha "${IDX}/history/x64.20260918-0000/SHA256SUMS")" ] &&
...
mv "${IDX}/assets/x64.20260918-0000/mica-x64-dev-20260918-0000.micaupd" "${IDX}/aside/"
...
mv "${IDX}/aside/mica-x64-dev-20260918-0000.micaupd" "${IDX}/assets/x64.20260918-0000/"
...
sed -i $'s/^\\(product\tx64-dev\t[^\t]*\t[^\t]*\t\\)1\t/\\19\t/' "${IDX}/history/mica.20260917-0000/mica-build.lock"
(cd "${IDX}/history/mica.20260917-0000" && sha256sum mica-build.lock mica-index.json >SHA256SUMS)
...
sed -i $'s/^\\(input\tmica-core\t[^\t]*\t\\).*/\\1'"$(printf '8%.0s' $(seq 64))"'/' "${IDX}/history/x64.20260918-0200/mica-build.lock"
(cd "${IDX}/history/x64.20260918-0200" && sha256sum mica-build.lock >SHA256SUMS)
...
rm -rf "${IDX}/history/x64.20260918-0200"
...
mv "${IDX}/aside/assets/x64-prod.20260916-0100" "${IDX}/assets/"
...
cp "${IDX}/aside/history/x64-prod.20260916-0100/mica-build.lock" "${IDX}/aside/history/x64-prod.20260916-0100/SHA256SUMS" "${IDX}/downloads/${C}/"
...
mkdir -p "${IDX}/history/x64.20260915-0000"
printf 'not a lock\n' >"${IDX}/history/x64.20260915-0000/mica-build.lock"
...
mv "${IDX}/history/x64.20260918-0000" "${IDX}/aside/history/"
...
rm -rf "${IDX}/history/x64.20260915-0000"
...
rm -rf "${IDX}/history/virt-arm64-dev.20260916-0300"
