...
d="$(mutate unknown-update-kind)"; set_key "${d}" x64-dev UPDATE_KINDS '"root"'
refuse "an update kind the board's images.tsv does not declare" "the update kind root is not declared" x64-dev MICA_PRODUCTS_DIR="${d}"
