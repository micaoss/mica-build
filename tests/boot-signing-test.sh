#!/usr/bin/env bash
# Authenticode signing is repeatable: boot/kernel.sh signs the systemd-boot loader twice with a throwaway
# key to the same bytes, and the signature validates against its certificate and not against another.
#
#   bash tests/boot-signing-test.sh      (make os-boot-test; after make os-boot-tools)
set -euo pipefail
cd "$(dirname "$0")/.."
REPO_ROOT="$(pwd)"
