...
The mini-x64 board (UEFI, systemd-boot, QEMU and generic PCs) of Mica OS: the board definition
(`board.env`), the kernel build (`kernel/`), the board package (`package/`) and the
board evidence (`evidence.json`). One board of `micaoss/mica-boards`; `make mini-x64-kernel`, `make pool` and
