...
const COMMANDS: Record<string, { module: string, what: string }> = {
  ...
  'from': { module: 'src/locks/from.ts', what: 'resolve an image selector of locks/ to its digest reference: --ref <selector>, <ARG>=<selector>..., --check' },
  'upstream': { module: 'src/locks/upstream.ts', what: 'one field of a git or source row of locks/upstream.lock' },
