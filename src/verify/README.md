...
| interpolation | `BOOT_SLOT_REQUIRED_FILES="Image rk3576-src.dtb ${BOOT_SCRIPT_NAME} mica-verity-@SLOT@.env"` |
...
```sh
bash bin/bun.sh src/cli.ts verify --board cx3576 --probe
```
...
bash bin/bun.sh src/cli.ts verify --board uefi-x64      # the other board
bash bin/bun.sh src/cli.ts verify --board uefi-x64 --image PATH
```
...
a contract rather than a preference: `tests/suites/apid-api` describes its own output as
...
```sh
bash bin/bun.sh src/cli.ts smoke                # uefi-x64, or $MICA_BOARD
bash bin/bun.sh src/cli.ts smoke --board cx3576
```
...
than a convenience. `.github/workflows/ci.yml` installs no bun for exactly
