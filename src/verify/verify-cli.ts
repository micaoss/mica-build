...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i]
    if (arg === '--help') {
      console.log('Usage: bin/bun.sh src/cli.ts verify --board BOARD --image FULL_FACTORY_IMAGE --public-key BASE64_KEY_FILE [--public-key FILE] [--work DIRECTORY]')
      return
    }
    const value = args[++i]
...
try { await main() }
catch (error) { console.error(`RESULT: FAIL: ${error instanceof Error ? error.message : String(error)}`); process.exitCode = 1 }
