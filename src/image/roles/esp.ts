// esp: the EFI System Partition a systemd-boot board boots from -- FAT32 with the loader tree, the boot entries
...
export const esp: RoleBuilder = {
  async build(ctx, partition, output) {
    ...
    await ctx.tb.must(['mkfs.vfat', '--invariant', '-F', '32', '-i', partition.volumeId!, '-n', 'MICAESP', output])
