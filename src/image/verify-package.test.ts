...
import { boardEnvPath, REPO_ROOT, SRC_DIR } from './paths.ts'
...
describe('build imports the model rather than owning a copy', () => {
  ...
  test('the imported parser is the shared board-env module', async () => {
    const direct = await import(join(REPO_ROOT, 'src', 'verify', 'board-env.ts'))
    expect(direct.parseBoardEnv).toBe(parseBoardEnv)
  })

  test('exactly one file in src/image imports from src/verify', () => {
