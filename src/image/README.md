...
`src/verify/board-env.ts` parses `boards/<board>/board.env` as **data** —
...
- the imported module **is** `src/verify/board.ts`, by object identity — bun
...
- **The mcopy order** — `Image rk3576-src.dtb boot.scr mica-verity-a.env
  mica-verity-b.env` — because that is the order they land in the FAT directory.
...
`src/toolbox.ts` is one function with two routes, which is `bin/bun.sh src/cli.ts`'s
...
**Identity mounts**, not `/w` or `/work`. `bin/bun.sh src/cli.ts` states the rule —
...
src/images.ts              an image key -> its reference, through from.sh and nothing else
