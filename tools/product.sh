...
PINNED_BOARDS="$(bash "${HERE}/../bin/bun.sh" src/cli.ts boards list)" || die "boards/boards.tsv could not be read (see above)"
printf '%s\n' "${PINNED_BOARDS}" | grep -Fx -- "${BOARD}" >/dev/null || die "product ${NAME}: BOARD=${BOARD} is not a pinned board; the pinned boards are: $(printf '%s ' ${PINNED_BOARDS})"
